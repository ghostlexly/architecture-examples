...
const directiveName = "isPublic";

/**
 * Block access to all fields that are not marked as @isPublic directive
 *
 * @param param0
 * @returns
 */
const isPublicDirectiveResolver = (schema: GraphQLSchema) => {
  return mapSchema(schema, {
    ...
    [MapperKind.OBJECT_TYPE]: (fieldConfig) => {
      ...
      if (
        !directive &&
        fieldConfig.name !== "Query" &&
        fieldConfig.name !== "Mutation"
      ) {
...
export { isPublicDirectiveResolver };
